// Array com todos os itens do cardápio
const menu = [
...
// Seleciona no HTML a área onde os itens do menu serão exibidos
const sectionCenter = document.querySelector(".section-center");

// Seleciona o container onde os botões de categoria serão inseridos
const btnContainer = document.querySelector(".btn-container");

// Evento que executa quando o DOM (HTML) termina de carregar
window.addEventListener("DOMContentLoaded", () => {
  displayMenuItems(menu); // Mostra todos os itens do menu inicialmente
  displayMenuButtons();   // Cria os botões de categorias
});

// Função responsável por renderizar os itens do cardápio na tela
function displayMenuItems(menuItems) {

  // Percorre cada item do menu e gera o HTML correspondente
  let displayMenu = menuItems.map((item) => {
    return `<article class="menu-item">
        <div class="item-info">
          <header>
            <h4>${item.title}</h4> <!-- Nome do prato -->
            <h4 class="price">R$ ${item.price.toFixed(2)}</h4> <!-- Preço formatado -->
          </header>
          <p class="item-text">${item.desc}</p> <!-- Descrição -->
        </div>
      </article>`;
  }).join(""); // Junta todos os itens em uma única string HTML

  // Insere o HTML gerado dentro da section do menu
  sectionCenter.innerHTML = displayMenu;
}

// Função que cria dinamicamente os botões de categoria
function displayMenuButtons() {

  // Cria uma lista de categorias únicas + botão "todos"
  const categories = ["todos", ...new Set(menu.map(item => item.category))];
  
  // Gera os botões no HTML com base nas categorias
  btnContainer.innerHTML = categories.map(category => {
    return `<button class="filter-btn" type="button" data-id="${category}">${category}</button>`;
  }).join("");

  // Seleciona todos os botões criados
  ...
  filterBtns.forEach(btn => {
    btn.addEventListener("click", (e) => {
      ...
      // Se clicar em "todos" mostra tudo, senão mostra apenas a categoria filtrada
